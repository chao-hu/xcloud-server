...
@Service
@Scope("prototype")
public class KafkaClusterDeleteWorker extends BaseKafkaClusterWorker {

    private static Logger LOG = LoggerFactory.getLogger(KafkaClusterDeleteWorker.class);

    @Override
    public void execute() {
        LOG.info("===============KafkaClusterDeleteWorker====================");
        // 1、获取参数
        ...
        // 3、获取kafkaCluster
        KafkaCluster kafkaCluster = componentOperationsClientUtil.getKafkaCluster(tenantName, service.getServiceName());
        if (null == kafkaCluster) {
            LOG.info("删除集群时kafka集群不存在，service" + JSON.toJSONString(service));
        ...
        if (!deleteAndRetry(tenantName, service.getServiceName())) {
            LOG.error("kafka集群:" + service.getServiceName() + "删除失败");
