...
@Service
@Scope("prototype")
public class MemcachedClusterDeleteWorker extends BaseMemcachedClusterWorker {

    private static Logger LOG = LoggerFactory.getLogger(MemcachedClusterDeleteWorker.class);

    @Override
    public void execute() {
        LOG.info("===============MemcachedClusterDeleteWorker====================");
        // 1、获取参数
        ...
        if (null == memcachedCluster) {
            LOG.info("删除集群时memcached集群不存在，service" + JSON.toJSONString(service));
...
//                    + service.getMemory() * (-1) + ",updateCapacity:" + service.getStorage() * (-1));
        ...
        if (!deleteResult) {
            ...
            LOG.error("memcached集群:" + service.getServiceName() + "删除失败");
        } else {
            componentOperationsDataBaseUtil.updateClusterAndNodesState(serviceId, CommonConst.STATE_CLUSTER_DELETED,
                    CommonConst.STATE_NODE_DELETED, null);
            componentOperationsClientUtil.deleteNodesLvm(tenantName, service.getId());
...
//                    + service.getMemory() * (-1) + ",updateCapacity:" + service.getStorage() * (-1));
    ...
    private boolean deleteAndRetry(String tenantName, String serviceName) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            boolean result = deleteMemcachedCluster(tenantName, serviceName);
            if (result) {
                return true;
            }
            try {
            ...
            if (i == 2) {
                LOG.error("删除memcached集群" + serviceName + "超过最大限制次数！");
                return false;
            }
        }
        return false;
