...
@Service
@Scope("prototype")
public class MysqlClusterChangeResourceWorker extends BaseMysqlClusterWorker {

    private static Logger LOG = LoggerFactory.getLogger(MysqlClusterChangeResourceWorker.class);

    @Override
    public void execute() {
        LOG.info("===============MysqlClusterChangeResourceWorker====================");
        // 1、获取参数
        ...
        while (true) {
            ...
            newCpu = newCluster.getSpec().getResources().getLimits().getCpu();
            newMemory = newCluster.getSpec().getResources().getLimits().getMemory();

            if (cpu.equals(newCpu) && memory.equals(newMemory) && newCluster.getStatus().isResourceupdateneedrestart()
                    && !newCluster.getStatus().isChangeResourceDoing()) {
                try {
                    service = componentOperationsDataBaseUtil.getStatefulServiceById(serviceId);
                    if (null != service) {
