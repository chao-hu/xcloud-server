...
@Service
@Scope("prototype")
public class PrometheusClusterDeleteWorker extends BasePrometheusClusterWorker {

    private static Logger LOG = LoggerFactory.getLogger(PrometheusClusterDeleteWorker.class);

    @Override
    public void execute() {
        LOG.info("===============PrometheusClusterDeleteWorker ====================");
        // 1、获取参数
        ...
        if (null == prometheusCluster) {
            LOG.info("删除集群时prometheus集群不存在，service" + JSON.toJSONString(service));
...
//                    + service.getMemory() * (-1) + ",updateCapacity:" + service.getStorage() * (-1));
        ...
        if (!deleteResult) {
            ...
            LOG.error("prometheus集群:" + service.getServiceName() + "删除失败");
        } else {
            componentOperationsClientUtil.deleteNodesLvm(tenantName, serviceId);
            componentOperationsDataBaseUtil.updateClusterAndNodesState(serviceId, CommonConst.STATE_CLUSTER_DELETED,
                    CommonConst.STATE_NODE_DELETED, null);
...
//                    + service.getMemory() * (-1) + ",updateCapacity:" + service.getStorage() * (-1));
    ...
    private boolean deleteAndRetry(String tenantName, String serviceName) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            boolean result = deletePrometheusCluster(tenantName, serviceName);
            if (result) {
                return true;
            }
            try {
            ...
            if (i == 2) {
                LOG.error("删除prometheus集群" + serviceName + "超过最大限制次数！");
                return false;
            }
        }
        return false;
