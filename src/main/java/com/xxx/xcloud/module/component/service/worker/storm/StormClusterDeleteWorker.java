...
@Service
@Scope("prototype")
public class StormClusterDeleteWorker extends BaseStormClusterWorker {

    private static Logger LOG = LoggerFactory.getLogger(StormClusterDeleteWorker.class);

    @Override
    public void execute() {
        LOG.info("===============StormClusterDeleteWorker====================");
        // 1、获取参数
        ...
        // 3、获取stormCluster
        StormCluster stormCluster = componentOperationsClientUtil.getStormCluster(tenantName, service.getServiceName());
        if (null == stormCluster) {
            LOG.info("删除集群时storm集群不存在，service" + JSON.toJSONString(service));
        ...
        if (!deleteAndRetry(tenantName, service.getServiceName())) {
            LOG.error("storm集群:" + service.getServiceName() + "删除失败");
...
//                + service.getMemory() * (-1) + ",updateCapacity: 0.0");
        // 删除依赖表
