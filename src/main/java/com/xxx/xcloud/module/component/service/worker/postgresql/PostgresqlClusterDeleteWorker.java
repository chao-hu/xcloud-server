...
@Service
@Scope("prototype")
public class PostgresqlClusterDeleteWorker extends BasePostgresqlClusterWorker {

    private static final Logger LOG = LoggerFactory.getLogger(PostgresqlClusterDeleteWorker.class);

    @Override
    public void execute() {
        LOG.info("===============PostgresqlClusterDeleteWorker====================");
        // 1、获取参数
        ...
        if (null == postgresqlCluster) {
            LOG.info("删除集群时postgresql集群不存在，service" + JSON.toJSONString(service));
            optClusterDelete(tenantName, service);
            return;
        }

        // 3、删除postgresqlCluster
        if (!deleteAndRetry(tenantName, service.getServiceName())) {
            LOG.error("postgresqlCluster集群:" + service.getServiceName() + "删除失败!");
