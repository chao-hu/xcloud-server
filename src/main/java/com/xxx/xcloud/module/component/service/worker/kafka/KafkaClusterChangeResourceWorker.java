...
@Service
@Scope("prototype")
public class KafkaClusterChangeResourceWorker extends BaseKafkaClusterWorker {

    private static Logger LOG = LoggerFactory.getLogger(KafkaClusterChangeResourceWorker.class);

    @Override
    public void execute() {
        LOG.info("===============KafkaClusterChangeResourceWorker====================");
        // 1、获取参数
        String serviceId = data.get("serviceId");
        String tenantName = data.get("tenantName");
        String cpu = data.get("cpu");
        String memory = data.get("memory") + CommonConst.UNIT_GI;
        String capacity = data.get("capacity") + CommonConst.UNIT_GI;
        ...
        while (true) {
            ...
            if (cpu.equals(newCluster.getSpec().getResources().getLimits().getCpu())
                    ...
                    && capacity.equals(newCluster.getSpec().getCapacity())
